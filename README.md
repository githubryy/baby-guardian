...
| `reminder_tasks` | `idx_nextRemindTime_enabled` | `nextRemindTime` + `enabled` | 复合 | ❌ 非唯一 | cron-scan 扫描，多个事项可能同一时间到期 |
| `reminder_tasks` | `idx_babyId` | `babyId` | 单字段 | ❌ 非唯一 | 一个宝宝可有多个提醒事项 |
| `reminder_tasks` | `idx_userId_enabled` | `userId` + `enabled` | 复合 | ❌ 非唯一 | v1.0 按用户查询事项 |
| `reminder_tasks` | `idx_familyId_enabled` | `familyId` + `enabled` | 复合 | ❌ 非唯一 | v2.0 按家庭查询事项 |
