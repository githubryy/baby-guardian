...
| 1 | `users` | 用户表 | 1 个 |
| 2 | `babies` | 宝宝表 | 1 个 |
| 3 | `reminder_tasks` | 提醒事项表（核心） | 4 个 |
| 4 | `confirm_logs` | 确认日志表 | 2 个 |
| 5 | `overdue_events` | 显著超时事件表 | 1 个 |
| 6 | `subscription_quotas` | 订阅配额表 | 1 个 |
| 7 | `notification_logs` | 推送日志表 | 1 个 |
| 8 | `families` | 家庭表（v2.0） | 2 个 |
| | **合计** | **8 个集合** | **13 个索引** |
...
| `endedAt` | string | 结束时间（ISO 8601），不存在表示活跃中 |
...
| 索引名 | 字段 | 类型 | 用途 |
|--------|------|------|------|
| `idx_nextRemindTime_endedAt` | `nextRemindTime` + `endedAt` | 复合索引 | cron 每分钟查询 `endedAt 不存在 && nextRemindTime <= now` |
| `idx_babyId` | `babyId` | 单字段 | 查某宝宝的所有事项 |
| `idx_userId_endedAt` | `userId` + `endedAt` | 复合索引 | 查某用户的活跃事项 |
...
| 索引名 | 字段 | 类型 | 用途 |
|--------|------|------|------|
| `idx_babyId_createdAt` | `babyId` + `createdAt` | 复合索引 | 统计页查某宝宝的历史记录 |

---

### 5. `overdue_events` — 显著超时事件表

`cron-scan` 检测到事项显著超时（超过 10 分钟未操作）时写入一条记录，用于统计按日精确计算显著超时次数。

| 字段 | 类型 | 说明 |
|------|------|------|
| `_id` | string | 自动生成 |
| `taskId` | string | 关联的事项 _id |
| `familyId` | string | 家庭 ID |
| `occurredAt` | string | 显著超时发生时间 |

---

### 6. `subscription_quotas` — 订阅配额表
...
| 集合 | 索引名 | 索引字段 | 类型 | 是否唯一 | 说明 |
|------|--------|---------|------|---------|------|
| `users` | `idx_openId` | `openId` | 单字段 | ✅ 唯一 | 每个微信用户 openId 唯一，防止重复注册 |
| `babies` | `idx_userId` | `userId` | 单字段 | ❌ 非唯一 | 一个用户可管理多个宝宝 |
| `reminder_tasks` | `idx_nextRemindTime_endedAt` | `nextRemindTime` + `endedAt` | 复合 | ❌ 非唯一 | cron-scan 扫描，多个事项可能同一时间到期 |
| `reminder_tasks` | `idx_babyId` | `babyId` | 单字段 | ❌ 非唯一 | 一个宝宝可有多个提醒事项 |
| `reminder_tasks` | `idx_userId_endedAt` | `userId` + `endedAt` | 复合 | ❌ 非唯一 | v1.0 按用户查询事项 |
| `reminder_tasks` | `idx_familyId_endedAt` | `familyId` + `endedAt` | 复合 | ❌ 非唯一 | v2.0 按家庭查询事项 |
| `confirm_logs` | `idx_babyId_createdAt` | `babyId` + `createdAt` | 复合 | ❌ 非唯一 | 按宝宝查询历史记录 |
| `confirm_logs` | `idx_familyId_createdAt` | `familyId` + `createdAt` | 复合 | ❌ 非唯一 | v2.0 按家庭查询历史记录 |
| `overdue_events` | `idx_familyId_occurredAt` | `familyId` + `occurredAt` | 复合 | ❌ 非唯一 | 按家庭统计显著超时 |
| `subscription_quotas` | `idx_openId_status_expireAt` | `openId` + `status` + `expireAt` | 复合 | ❌ 非唯一 | 一个用户可有多条配额记录 |
| `notification_logs` | `idx_taskId_sendTime` | `taskId` + `sendTime` | 复合 | ❌ 非唯一 | 一个事项可有多条推送日志 |
| `families` | `idx_inviteCode` | `inviteCode` | 单字段 | ✅ 唯一 | 邀请码全局唯一，确保加入正确家庭 |
| `families` | `idx_ownerUserId` | `ownerUserId` | 单字段 | ✅ 唯一 | 一个用户只能创建一个家庭 |

> **创建方式**：云开发控制台 → 数据库 → 选中集合 → 索引管理 → 添加索引。复合索引需按上表顺序添加字段。
>
> **唯一索引创建方法**：添加索引时勾选「**唯一**」选项。唯一索引可防止重复数据插入，例如同一 openId 注册多个用户、同一邀请码对应多个家庭。
>
> **唯一索引注意事项**：
> - `users.idx_openId`：确保每个微信用户只有一条记录，登录时通过 `where({ openId }).limit(1)` 查询
> - `families.idx_inviteCode`：确保邀请码不重复，加入家庭时通过 `where({ inviteCode }).limit(1)` 查询
> - `families.idx_ownerUserId`：确保一个用户只能创建一个家庭，创建前会检查 `existingFamily`
