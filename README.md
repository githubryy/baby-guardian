...
| 5 | `subscription_quotas` | 订阅配额表 | 1 个 |
| 6 | `notification_logs` | 推送日志表 | 1 个 |
| 7 | `families` | 家庭表（v2.0） | 2 个 |
| | **合计** | **7 个集合** | **12 个索引** |
...
| `confirm_logs` | `idx_babyId_completedTime` | `babyId` + `completedTime` | 复合 | ❌ 非唯一 | 按宝宝查询历史记录 |
| `confirm_logs` | `idx_familyId_completedTime` | `familyId` + `completedTime` | 复合 | ❌ 非唯一 | v2.0 按家庭查询历史记录 |
